...
function extendSpanData () {
  ...
  BaseSpanData.prototype.addAppInfo = function (name, version) {
    const labels = {}
    labels[CONST.TRACE_LABELS.GAE_MODULE_NAME] = name
    labels[CONST.TRACE_LABELS.GAE_MODULE_VERSION] = version
  ...
  BaseSpanData.prototype.createChildSpanData = function (name, skipFrames) {
    ...
    if (!rootSpan) {
      ...
      if (process.env.ENABLE_GCLOUD_WARNINGS) console.warn('created child span for a closed root span')
      return uncorrelatedSpanData
    }
  ...
  const uncorrelatedSpanData = Object.assign(Object.create(BaseSpanData.prototype), {
    createChildSpanData: function () {
      return uncorrelatedSpanData
    }
  }, UNCORRELATED_SPAN)
}
